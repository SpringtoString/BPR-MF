...
void hit(const vector<int> &rank, const unordered_set<int> &truth, float *result_pt){
    int hits = 0;
    for(unsigned int i=0; i<rank.size(); i++)
    {
        if(hits==0 && truth.find(rank[i]) != truth.end())
        {
            hits = 1;
        }
        if(hits) result_pt[i] = 1.0;
    }
}
...
unordered_map<int, metric_fun> metric_dict = {{0, precision},
                                              {1, recall},
                                              {2, ndcg},
                                              {3, hit},
                                              {4, ap},
