...
class MF(nn.Module):
    ...
    def fit(self, learning_rate=0.001, batch_size=500, epochs=15, verbose=5, early_stop=False):
        ...
        prec_list,rec_list,ndcg_list,hr_list,ap_list=[],[],[],[],[]
        ...
        for epoch in range(epochs):
            ...
            if epoch%verbose==0 or epoch==epochs-1:
                start_time = time.time()
                result = self.test(batch_size=2*batch_size)
                eval_time = time.time() - start_time

                eval_res = PrettyTable()
                eval_res.field_names = ['epoch','time','precision','recall','ndcg','hit_ratio','MAP']
                eval_res.add_row([epoch, eval_time,result['precision'],result['recall'],result['ndcg'],result['hit_ratio'],result['MAP']])
                print(eval_res)

                prec_list.append(result['precision'][0])
                rec_list.append(result['recall'][0])
                ndcg_list.append(result['ndcg'][0])
                hr_list.append(result['hit_ratio'][0])
                ap_list.append(result['MAP'][0])

            print(" ")

        '''保存测试指标结果'''
        df = pd.DataFrame(
            {'precision': prec_list, 'recall': rec_list, 'ndcg': ndcg_list, 'hr': hr_list, 'MAP': ap_list})
        df.to_csv('%s.csv' % (self.moudle_name), index=False)
    ...
    def test(self, batch_size=256, ):
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            rate_batch = model.rating(user_batch,all_item).detach().cpu()  # shape is [len(user_batch),ITEM_NUM] 即预测评分矩阵
